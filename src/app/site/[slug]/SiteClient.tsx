...
export default function SiteClient({ data }: { data: SiteData }) {
  ...
  const [activeSection, setGctiveSection] = useState('hero-section')
  useEffect(() => {
    const fn = () => {
  ...
  /* ── Category tab ── */
  const [activeCat, setGctiveCat] = useState(0)
  ...
  const extractQuantityGndQuery = (raw: string): { qty: number; query: string } => {
  ...
  const tryOrderFromText = useCallback((text: string): boolean => {
    ...
    for (const part of parts) {
      const { qty, query } = extractQuantityGndQuery(part)
    ...
    if (added.length === 0) {
      const { qty, query } = extractQuantityGndQuery(text)
      const item = findMenuItem(query)
...
¿Qué te gustaría hacer?`, 'options', [
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-green-500/30 overflow-x-hidden">
            ...
            <h1 className="relative mb-3 sm:mb-6">
              ...
              <span
                ...
                style={{
                  color: G,
                  fontFamily: 'Georgia, "Times New Roman", serif',
                  textShadow: `0 0 16px ${G}22, 0 1px 0 #b45309`,
                }}
              >
                Smash Burgers
              </span>
      ...
      {/* ═══════ STATS CARDS ═══════ */}
      <section className="relative z-20 max-w-5xl mx-auto px-4 sm:px-8 py-6 sm:py-12">
        <div className="grid grid-cols-3 gap-2 sm:gap-5">
            ...
            <div key={i} className="group relative p-2.5 sm:p-6 rounded-xl sm:rounded-2xl border border-white/[0.06] bg-[#111]/80 backdrop-blur-xl text-center hover:border-white/[0.15] transition-all duration-700 hover:translate-y-[-3px]" style={{ boxShadow: 'inset 0 1px 0 0 rgba(255,255,255,0.04), 0 8px 40px rgba(0,0,0,0.5)' }}>
              ...
              {s.icon ? (
                <s.icon size={16} className="mx-auto mb-1 sm:mb-2 transition-transform duration-500 group-hover:scale-110 relative z-10" style={{ color: G }} />
              ) : (
                <div className="h-[16px] sm:h-[18px] mb-1 sm:mb-2" />
              )}
              <p className="text-base sm:text-3xl font-black relative z-10 text-white">{s.val}</p>
      ...
      <section id="order-section" className="py-10 sm:py-20 relative overflow-hidden scroll-mt-20">
        ...
        <div className="absolute top-0 left-0 right-0 flex justify-center gap-1 py-3 hidden sm:flex">
          {[...Array(40)].map((_, i) => (
            <div key={i} className="w-2 h-2 rotate-45 shrink-0" style={{ background: i % 2 === 0 ? `${G}10` : 'transparent' }} />
          ))}
        </div>
        <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
          <div className="text-center mb-6 sm:mb-12">
            ...
            <h2 className="text-xl sm:text-4xl lg:text-5xl font-black leading-[1] sm:leading-[0.9] tracking-tight">
              &iquest;C&Oacute;MO POD&Eacute;S<br /><span style={{ color: G }}>PEDIR EN QUINCHO&apos;S?</span>
            </h2>
            <p className="text-white/40 mt-3 sm:mt-6 max-w-xl mx-auto text-xs sm:text-base leading-relaxed">Escog&eacute; tu opci&oacute;n favorita para el pedido</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 max-w-2xl mx-auto">
            {[
              ...
              <div key={i} className="group relative p-5 sm:p-8 rounded-2xl border border-white/[0.06] bg-white/[0.015] hover:bg-white/[0.03] hover:border-white/[0.12] transition-all duration-500 text-center" style={{ boxShadow: 'inset 0 1px 0 0 rgba(255,255,255,0.03)' }}>
                <div className="absolute top-3 right-3 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black border" style={{ borderColor: `${opt.color}25`, color: `${opt.color}60` }}>
                  0{i + 1}
                </div>
                <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center mx-auto mb-3 sm:mb-5 transition-all duration-500 group-hover:scale-110" style={{ background: `${opt.color}12`, border: `1px solid ${opt.color}20` }}>
                  <opt.icon size={20} className="sm:w-[26px] sm:h-[26px]" style={{ color: opt.color }} />
                </div>
                <h3 className="text-sm sm:text-xl font-black mb-1.5 sm:mb-2 uppercase tracking-wide">{opt.title}</h3>
                <p className="text-white/40 text-[11px] sm:text-sm leading-relaxed mb-3 sm:mb-6">{opt.desc}</p>
                <button onClick={opt.action} className="w-full py-3 sm:py-4 rounded-xl text-xs sm:text-sm font-black uppercase tracking-wider text-white transition-all hover:brightness-125 active:scale-[0.98] min-h-[44px] sm:min-h-[48px]" style={{ background: `${opt.color}20`, border: `1px solid ${opt.color}30` }}>
                  {opt.btn}
                </button>
              </div>
            ))}
      ...
      {/* ═══════ BRAND DIVIDER ═══════ */}
      <div className="flex items-center justify-center gap-3 py-6 sm:py-6">
        <div className="h-px flex-1 max-w-[120px]" style={{ background: `linear-gradient(90deg, transparent, ${G}30)` }} />
        <div className="flex gap-1.5">
          {[...Array(7)].map((_, i) => (
            <div key={i} className="w-3 h-3 rotate-45" style={{ background: i % 2 === 0 ? `${G}25` : 'transparent', border: `1px solid ${G}18` }} />
          ))}
        </div>
      ...
      {/* ═══════ ABOUT — Minimal ═══════ */}
      {hasMenu && (
        <section id="about-section" className="relative py-10 sm:py-20 overflow-hidden scroll-mt-20">
          ...
          <div className="relative max-w-5xl mx-auto px-4 sm:px-8 text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-5">
              {[
                ...
                <div key={i} className="flex flex-col items-center gap-2 p-4 sm:p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/[0.10] transition-all duration-300">
                  ...
                  <p className="text-xs sm:text-base font-bold text-white/85">{item.label}</p>
                  <p className="text-[10px] sm:text-xs text-white/35">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="locations-section" className="py-8 sm:py-20 relative overflow-hidden scroll-mt-20">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-8">
            <div className="text-center mb-5 sm:mb-14">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/[0.06] text-[11px] font-bold uppercase tracking-[0.25em] mb-2 sm:mb-4" style={{ color: G }}>
                <MapPin size={13} /> Locales
              </div>
              <h2 className="text-lg sm:text-4xl lg:text-5xl font-black leading-[0.9] tracking-tight">VISIT&Aacute;NOS<br /><span style={{ color: G }}>HORARIOS</span></h2>
            </div>
                ...
                return (
                  <div key={locKey} className="group p-3 sm:p-8 rounded-2xl sm:rounded-3xl border border-white/[0.06] bg-white/[0.015] hover:border-white/[0.12] transition-all duration-500 hover:translate-y-[-4px]" style={{ boxShadow: 'inset 0 1px 0 0 rgba(255,255,255,0.03), 0 4px 30px rgba(0,0,0,0.2)' }}>
                    <div className="flex gap-1 mb-3 sm:mb-5 hidden sm:flex">
                      {[...Array(8)].map((_, j) => (
                        <div key={j} className="w-2.5 h-2.5 rotate-45 transition-all duration-500" style={{ background: j % 2 === 0 ? `${G}25` : 'transparent', border: `1px solid ${G}14` }} />
                      ))}
                    </div>
                    <div className="flex items-start gap-2.5 sm:gap-4 mb-3 sm:mb-6">
                      <div className="w-10 h-10 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl flex items-center justify-center shrink-0 transition-transform duration-500 group-hover:scale-110" style={{ background: `${G}10`, border: `1px solid ${G}15` }}>
                        <MapPin size={18} className="sm:w-6 sm:h-6" style={{ color: G }} />
                      </div>
                      <div className="min-w-0">
                        <h3 className="font-black text-base sm:text-2xl uppercase">Quincho&apos;s</h3>
                    ...
                    <div className="space-y-1.5 sm:space-y-3 pt-2.5 sm:pt-5 border-t border-white/[0.06]">
                      {displayHours && (
                        <div className="flex items-start gap-2.5">
                          <Clock size={14} className="mt-0.5 shrink-0 sm:w-4 sm:h-4" style={{ color: G }} />
                      ...
                      <div className="flex items-center gap-2.5">
                        <Truck size={14} className="sm:w-4 sm:h-4" style={{ color: G }} />
      ...
      {/* ═══════ CTG — Visual banner ═══════ */}
      <section className="max-w-7xl mx-auto px-4 sm:px-8 py-4 sm:py-16">
        <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl border border-white/[0.06]" style={{ background: `linear-gradient(135deg, rgba(245,158,11,0.05), #0a0a0a, rgba(106,191,75,0.03))` }}>
          {/* Warm glows */}
          <div className="absolute top-0 left-0 w-[300px] sm:w-[500px] h-[300px] sm:h-[500px] rounded-full blur-[200px] opacity-[0.06]" style={{ background: G }} />
          ...
          <div className="relative grid sm:grid-cols-2 items-center">
            {/* Text side */}
            <div className="relative p-5 sm:p-12 lg:p-16 z-10">
              <div className="flex gap-1.5 mb-5 sm:mb-8 hidden sm:flex">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="w-3 h-3 rotate-45" style={{ background: i % 2 === 0 ? `${G}35` : 'transparent', border: `1px solid ${G}20` }} />
                ))}
              </div>
              <h2 className="text-xl sm:text-4xl lg:text-5xl font-black leading-[0.9] mb-2 sm:mb-5 tracking-tight">
                SABOR QUE<br />SE SIENTE<br /><span style={{ color: G }}>EN CADA MORDIDA</span>
              </h2>
              <p className="text-white/40 max-w-md text-[11px] sm:text-base leading-relaxed">Carne 100% Angus aplastada en plancha caliente, queso derretido y pan brioche artesanal. As&iacute; es Quincho&apos;s.</p>
            </div>

            {/* Image side */}
            <div className="relative h-44 sm:h-full min-h-[200px] sm:min-h-[360px]">
              <img
                src={heroSlides[0]}
      ...
      {/* ═══════ CONTACT ═══════ */}
      <section id="contacto" className="max-w-7xl mx-auto px-4 sm:px-8 py-4 sm:py-16 scroll-mt-20">
        <div className="max-w-xl mx-auto">
          <div className="text-center mb-4 sm:mb-14">
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/5 border border-white/[0.06] text-[11px] font-bold uppercase tracking-[0.25em] mb-3 sm:mb-5 text-white/40">
              <Mail size={13} /> Contacto
            </div>
            <h2 className="text-lg sm:text-4xl font-black mb-1.5 tracking-tight">PEDIDOS<br /><span style={{ color: G }}>ESPECIALES</span></h2>
            <p className="text-white/30 text-[11px] sm:text-base mt-1.5 sm:mt-3">D&eacute;janos tus datos y te escribimos</p>
          </div>
          {formSent ? (
            <div className="text-center p-10 rounded-3xl border border-white/[0.06] bg-white/[0.02]">
              <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: `${G}15` }}>
                <CheckCircle2 size={28} style={{ color: G }} />
              </div>
              <h3 className="text-lg font-black mb-1.5">&iexcl;Mensaje enviado!</h3>
              <p className="text-white/40 text-sm">Te contactaremos pronto.</p>
            </div>
          ) : (
            <form onSubmit={handleFormSubmit} className="space-y-2.5 sm:space-y-4 p-4 sm:p-10 rounded-2xl sm:rounded-3xl border border-white/[0.06] bg-white/[0.015]">
              {formError && <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm">{formError}</div>}
              <div>
                <label className="block text-xs sm:text-sm text-white/50 mb-1.5 font-medium">Nombre *</label>
                <input type="text" required value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Tu nombre" className="w-full px-3.5 py-3 sm:py-3.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-white/20 text-sm focus:outline-none focus:border-white/20 transition-colors" />
              </div>
              <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  <label className="block text-xs sm:text-sm text-white/50 mb-1.5 font-medium">Email</label>
                  <input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} placeholder="tu@email.com" className="w-full px-3.5 py-3 sm:py-3.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-white/20 text-sm focus:outline-none focus:border-white/20 transition-colors" />
                </div>
                <div>
                  <label className="block text-xs sm:text-sm text-white/50 mb-1.5 font-medium">Tel&eacute;fono *</label>
                  <input type="tel" required value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} placeholder="+506 6012 3456" className="w-full px-3.5 py-3 sm:py-3.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-white/20 text-sm focus:outline-none focus:border-white/20 transition-colors" />
                </div>
              </div>
              <div>
                <label className="block text-xs sm:text-sm text-white/50 mb-1.5 font-medium">Mensaje</label>
                <textarea rows={3} value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} placeholder="&iquest;En qu&eacute; podemos ayudarte?" className="w-full px-3.5 py-3 sm:py-3.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-white/20 text-sm focus:outline-none focus:border-white/20 resize-none transition-colors" />
              </div>
              <button type="submit" disabled={formLoading} className="w-full py-3.5 sm:py-4 rounded-xl text-sm font-bold text-black transition-all hover:brightness-110 active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 min-h-[46px] sm:min-h-[48px]" style={{ background: ORANGE }}>
      ...
      {/* ═══════ FOOTER — Compact ═══════ */}
      <footer className="border-t border-white/[0.06] py-4 sm:py-8">
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full sm:max-w-md mx-0 sm:mx-4 rounded-t-2xl sm:rounded-3xl bg-[#111] border-t sm:border border-white/[0.08] shadow-2xl overflow-hidden max-h-[90vh] sm:max-h-[85vh] flex flex-col">
            <div className="px-5 py-4 sm:px-6 sm:py-5 border-b border-white/[0.06] flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2.5 sm:gap-3">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center" style={{ background: `${G}15` }}>
                  <ShoppingCart size={16} className="sm:w-[18px] sm:h-[18px]" style={{ color: G }} />
                </div>
                <div>
                  <h3 className="font-black text-base sm:text-lg">Tu orden</h3>
                  <p className="text-[11px] sm:text-xs text-white/40">{cartCount} {cartCount === 1 ? 'item' : 'items'}</p>
                </div>
              </div>
              <button onClick={() => setCartOpen(false)} className="p-2 rounded-xl hover:bg-white/[0.06] transition-colors"><X size={18} className="sm:w-5 sm:h-5 text-white/40" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-3 sm:p-5 space-y-2.5 sm:space-y-3">
              {cart.length === 0 ? (
                <div className="text-center py-10 sm:py-12 text-white/30">
                  <ShoppingCart size={28} className="sm:w-8 sm:h-8 mx-auto mb-3 opacity-30" />
                  <p className="text-sm">Tu carrito est&aacute; vac&iacute;o</p>
                </div>
              ) : cart.map((item, i) => (
                <div key={i} className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-0.5 bg-white/[0.06] rounded-xl p-1">
                    <button onClick={() => updateQty(item.name, -1)} className="w-8 h-8 sm:w-7 sm:h-7 rounded-lg flex items-center justify-center hover:bg-white/10 active:bg-white/15"><Minus size={13} className="sm:w-3 sm:h-3" /></button>
                    <span className="w-6 sm:w-6 text-center text-sm font-bold">{item.qty}</span>
                    <button onClick={() => updateQty(item.name, 1)} className="w-8 h-8 sm:w-7 sm:h-7 rounded-lg flex items-center justify-center hover:bg-white/10 active:bg-white/15"><Plus size={13} className="sm:w-3 sm:h-3" /></button>
                  </div>
                  <div className="text-right shrink-0 w-16 sm:w-20">
                    <p className="font-black text-sm">{fmt(item.price * item.qty)}</p>
                  </div>
                  <button onClick={() => removeFromCart(item.name)} className="p-1.5 rounded-lg hover:bg-red-500/10"><Trash2 size={13} className="sm:w-3.5 sm:h-3.5 text-red-400/60" /></button>
                </div>
              ))}
            </div>
            {cart.length > 0 && (
              <div className="p-4 sm:p-5 border-t border-white/[0.06] shrink-0 space-y-3 sm:space-y-4" style={{ paddingBottom: 'max(1.25rem, env(safe-area-inset-bottom))' }}>
                ...
                {orderSent ? (
                  <div className="flex items-center justify-center gap-2 py-3.5 sm:py-4 rounded-2xl bg-[#25D366]/10 border border-[#25D366]/20 text-[#25D366] font-bold text-sm">
                    <CheckCircle2 size={18} /> &iexcl;Orden enviada!
                  </div>
                ) : (
                  <button onClick={sendOrderWG} className="w-full py-3.5 sm:py-4 rounded-2xl text-sm font-black text-white transition-all hover:brightness-110 active:scale-[0.98] flex items-center justify-center gap-2.5 shadow-lg bg-[#25D366] min-h-[46px] sm:min-h-[48px]">
                    <Phone size={18} /> Pedir por WhatsApp
                  </button>
                )}
                <p className="text-center text-[10px] sm:text-[11px] text-white/20">Se abrir&aacute; WhatsApp con tu orden</p>
              </div>
            )}
          </div>
        </div>
      ...
      <button onClick={() => setChatOpen(prev => !prev)} className={`fixed bottom-5 right-5 z-[80] w-12 h-12 rounded-full shadow-2xl hidden sm:flex items-center justify-center transition-all hover:scale-105 active:scale-95 ${chatOpen ? '' : ''}`} style={{ background: ORANGE, boxShadow: `0 4px 20px ${ORANGE}50` }}>
      ...
      {chatOpen && (
        <div className="fixed inset-0 z-[70] sm:inset-auto sm:bottom-24 sm:right-5 sm:w-[380px] sm:h-[520px] sm:rounded-2xl border-0 sm:border border-white/[0.08] bg-[#0f0f0f] shadow-2xl flex flex-col overflow-hidden">
          <div className="px-3.5 py-2.5 sm:py-3.5 border-b border-white/[0.06] flex items-center gap-2.5 sm:gap-3 shrink-0 bg-[#0a0a0a]" style={{ paddingTop: 'max(0.625rem, env(safe-area-inset-top))' }}>
            <div className="relative">
              <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center" style={{ background: `${G}20` }}><Bot size={16} className="sm:w-[18px] sm:h-[18px]" style={{ color: G }} /></div>
              <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-green-500 border-2 border-[#0a0a0a]" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[13px] sm:text-sm font-bold truncate">Asistente Quincho&apos;s</p>
              <p className="text-[10px] sm:text-[11px] text-green-400/80">En línea • Consultas, horarios y ayuda rápida</p>
            </div>
            <button onClick={() => setChatOpen(false)} className="p-2 rounded-lg hover:bg-white/[0.06]"><X size={16} className="sm:w-[18px] sm:h-[18px] text-white/50" /></button>
          </div>
          ...
          {showInput && (
            <div className="p-2.5 sm:p-3 border-t border-white/[0.06] shrink-0" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
              <form onSubmit={(e) => { e.preventDefault(); handleStructuredInput() }} className="flex gap-2">
                <input ref={inputRef} type={phase === 'collect-email' ? 'email' : phase === 'collect-phone' ? 'tel' : 'text'} value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder={getPlaceholder()} className="flex-1 px-3.5 py-2.5 sm:py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-[16px] sm:text-sm placeholder:text-white/25 focus:outline-none focus:border-white/20" />
                <button type="submit" disabled={chatLoading || !chatInput.trim()} className="px-5 sm:px-4 py-3 sm:py-3.5 rounded-xl text-black transition-all hover:brightness-110 active:scale-95 disabled:opacity-30 font-bold min-w-[44px] sm:min-w-[48px]" style={{ background: G }}><ArrowRight size={18} className="sm:w-4 sm:h-4" /></button>
              </form>
            </div>
          )}
        </div>
